...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void newApple(){
        ...
        appleY = random.nextInt((int)(SCREEN_WIDHT/UNIT_SIZE))* UNIT_SIZE;
    ...
    public void checkCollisions(){
        //checa se a cabeça bateu no corpo
        for (int i = bodyParts; i<0; i--){
            if ((x[0]) == x[i ]&& y[0] == y[i]){
                running = false;
            }
        }
        //checa se a cabeça toca a borda
        if (x[0] < 0){
            running = false;
        }
        if (x[0] > SCREEN_WIDHT){
    ...
    public void gameOver(Graphics g){
        ...
        g.setFont(new Font("Ink Free",Font.BOLD,40));
        FontMetrics metrics2 = getFontMetrics(g.getFont());
        g.drawString("Score: "+applesEaten, (SCREEN_WIDHT - metrics2.stringWidth("Score"))/2,g.getFont().getSize());
